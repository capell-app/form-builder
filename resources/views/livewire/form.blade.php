    ...
    @if ($form)
        ...
        @else
            ...
            >
                        ...
                        <div class="capell-form__field space-y-2">
                            ...
                            @if ($field->type === FormFieldType::Textarea)
                                <textarea
                                    wire:model="data.{{ $field->key }}"
                                    id="{{ $fieldId }}"
                                    class="capell-form__control block min-h-28 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-900/10"
                            ...
                            @elseif ($field->type === FormFieldType::Select)
                                <select
                                    wire:model="data.{{ $field->key }}"
                                    id="{{ $fieldId }}"
                                    class="capell-form__control block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-gray-900 focus:outline-none focus:ring-2 focus:ring-gray-900/10"
                                    @if ($describedBy !== '') aria-describedby="{{ $describedBy }}" @endif
                                    aria-invalid="{{ $errors->has($errorKey) ? 'true' : 'false' }}"
                                    @required($field->required)
                                >
                            ...
                            @elseif ($field->type === FormFieldType::Checkbox)
                                <input
                                    ...
                                    wire:model="data.{{ $field->key }}"
                                    id="{{ $fieldId }}"
                                    class="capell-form__checkbox h-4 w-4 rounded border-gray-300 text-gray-900 focus:ring-gray-900/20"
                                    @if ($describedBy !== '') aria-describedby="{{ $describedBy }}" @endif
                                    aria-invalid="{{ $errors->has($errorKey) ? 'true' : 'false' }}"
                                    @required($field->required)
                                />
                ...
                <button
                    type="submit"
                    class="capell-form__submit inline-flex min-h-10 items-center justify-center rounded-md bg-gray-950 px-4 py-2 text-sm font-semibold text-white transition hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-60"
